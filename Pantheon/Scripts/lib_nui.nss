...
// nwnx_player stripped for open-source release; PlaySound replaces NWNX_Player_PlaySound below.
    ...
    float fWindowH = 0.0)
{
    ...
    json jRow = JsonArray();
    {
        json jNuiImage = NuiImage(
            JsonString(sGifResRef),
    ...
    if(sSoundResRef != "")
    {
        AssignCommand(oPC, PlaySound(sSoundResRef));
    }
    ...
    float fScaleMax = 1.5)
{
    ...
    else
    {
        jRow = JsonArrayInsert(jRow, NuiHeight(
            NuiSpacer(),
            GetNuiScaleDimension(oPC, fHeight)));
    }

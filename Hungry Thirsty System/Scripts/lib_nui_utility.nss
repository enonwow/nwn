...
// lib_nui_utility.nss
// =============================================================================
// Lower-level helpers used across NUI windows. Includes:
//   - Encouraged-row tracking helpers (NuiOffEncouragedData / NuiOnEncouragedData)
//   - Item icon resolution (GetIconResref, complex icon parts)
//   - Item property description formatting
//   - PC class lookup helpers
//   - Anti-exploit cutscene paralyze helpers
//   - Misc utilities (IntToPaddedString, GetHeight, debug message broadcast)
// =============================================================================
...
string GetItemPropertyDescription(itemproperty ip)
{
    ...
    if(subTypeTable != "")
    {
        ...
        if(nStrref > 0) subType = GetStringByStrRef(nStrref);
    }
    ...
    if(costValueTableId != "")
    {
        ...
        if(nStrref > 0) costValue = GetStringByStrRef(nStrref);
    }
    ...
    if(paramTableId != "")
    {
        ...
        if(nStrref > 0) paramValue = GetStringByStrRef(nStrref);
    }
    return propName
        + (subType != ""? " " + subType : "")
        + (costValue != ""? " " + costValue : "")
...
string GetClassFromJson(json jClasses, int nId)
{
    ...
    if (JsonGetType(jClass) == JSON_TYPE_NULL) return "";
...
string IntToPaddedString(int nX, int nLength = 4, int nSigned = FALSE)
{
    ...
    if(nSigned)
    {
        if(nX >= 0) sResult = "+" + sResult;
        else        sResult = "-" + sResult;
    }
    return sResult;
}

void GetNextPreviousValueFromCombo(object oPC, int nToken,
    string sBindId, string sBindEntries, int nMax, int nPlus)
{
    int nId = JsonGetInt(NuiGetBind(oPC, nToken, sBindId)) + nPlus;
    int nLnegth = JsonGetLength(NuiGetBind(oPC, nToken, sBindEntries));

    if(nMax > nLnegth) nMax = nLnegth - 1;

    if(nId < 0)         nId = nMax;
    else if(nId > nMax) nId = 0;
...
int GetHeight(object oPC)
{
    ...
    if (GetGender(oPC) == GENDER_FEMALE) fGender = 0.93;
